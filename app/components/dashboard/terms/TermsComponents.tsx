...
export const TermsHeader: React.FC<TermsHeaderProps> = ({ meta, isDark }) => {
  return (
    <div className={`border-b pb-6 transition-colors duration-200 ${isDark ? 'border-slate-700' : 'border-slate-300'}`}>
      ...
      <p className={` mt-3  text-base sm:text-md max-w-7xl leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
        Welcome to CareerMapFinder. By accessing our platform, 
        utilizing our AI Career Roadmap Generator, subscribing to Tiered Plans, 
        or engaging with our career analytics, you agree to these legally binding terms.
      </p>
        ...
        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${
          ...
          v{meta.version} Standard Agreement
        </span>
      </div>
...
export const TermsSidebar: React.FC<TermsSidebarProps> = ({
  sections,
  activeSection,
  onSelectSection,
  isDark
  ...
  return (
    <nav className="space-y-1.5 sticky top-24">
      <p className={`text-xs font-bold uppercase tracking-wider px-3 
        mb-3 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
        Table of Contents
      </p>
        ...
        return (
          <button
            ...
            className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg
               text-xs sm:text-sm font-medium transition-all duration-150 text-left ${
              isActive
                ? isDark
...
export const TermsCallout: React.FC<CalloutProps> = ({ title, text, type, isDark }) => {
  const getStyles = () => {
    switch (type) {
      case 'warning':
        return {
          bg: isDark ? 'bg-amber-950/40 border-amber-700 text-amber-200' : 'bg-amber-50 border-amber-200 text-amber-900',
          icon: AlertTriangle,
          iconColor: isDark ? 'text-amber-400' : 'text-amber-600',
        };
      case 'security':
        return {
          bg: isDark ? 'bg-emerald-950/40 border-emerald-700 text-emerald-200' : 'bg-emerald-50 border-emerald-200 text-emerald-900',
          icon: ShieldAlert,
      ...
      default:
        return {
          bg: isDark ? 'bg-indigo-950/40 border-indigo-700 text-indigo-200' : 'bg-indigo-50 border-indigo-200 text-indigo-900',
          icon: FileCheck2,
...
export const TermsSectionCard: React.FC<TermsSectionCardProps> = ({ section, isDark }) => {
  const Icon = section.icon;

  return (
    <section 
      id={section.id} 
      className={`p-6 sm:p-8 rounded-2xl border transition-colors duration-200 ${
        isDark ? 'bg-slate-900/60 border-slate-700' : 'bg-white border-slate-300 shadow-sm'
      }`}
    >
      <div className="flex items-center gap-3 mb-4">
        <div className={`p-2.5 rounded-xl ${
          isDark ? 'bg-indigo-950 text-indigo-400 border border-indigo-700' 
